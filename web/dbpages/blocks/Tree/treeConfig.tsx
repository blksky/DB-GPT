import { DatabaseTypeCode, OperationColumn, TreeNodeType } from '@/dbpages/constants';
import connectionService from '@/dbpages/service/connection';
import { IConnectionDetails, ITreeNode } from '@/dbpages/typings';
import { v4 as uuid } from 'uuid';

import mysqlServer from '@/dbpages/service/sql';
    ...
    getChildren: (params, options) => {
      ...
      return new Promise((r, j) => {
        mysqlServer
          ...
          .then(res => {
            let tableList: ITreeNode[] = res.data?.map((t: any) => {
              return {
                uuid: uuid(),
                name: t.name,
            ...
            if (params.databaseType === DatabaseTypeCode.SQLITE) {
              tableList = tableList.filter(
                d => d.name !== 'sqlite_sequence' && !d.name.startsWith('sqlite_autoindex_'),
              );
            }
            r({
              data: tableList,
