import { IChartItem, IDashboardItem, IPageResponse } from '@/dbpages/typings';
import createRequest from './base';

/** 获取报表列表 */
const getDashboardList = createRequest<{}, IPageResponse<IDashboardItem>>('/api/dashboard/list', {
  method: 'get',
});
const getDashboardById = createRequest<{ id: number }, IDashboardItem>('/api/dashboard/:id', {
  method: 'get',
});
/** 创建报表 */
...
/** 更新报表 */
const updateDashboard = createRequest<IDashboardItem, void>('/api/dashboard/update', {
  method: 'post',
});
/** 删除报表 */
const deleteDashboard = createRequest<{ id: number }, string>('/api/dashboard/:id', {
  method: 'delete',
});
