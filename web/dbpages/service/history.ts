...
// import { IPageResponse,IPageParams,IHistoryRecord, IWindowTab, ISavedConsole } from '@/pages/dbpages/types';
import { IConsole, ICreateConsole, IPageParams, IPageResponse } from '@/dbpages/typings';
import { ConsoleStatus, DatabaseTypeCode } from '../constants';
...
const createConsole = createRequest<ICreateConsole, number>('/api/operation/saved/create', {
  method: 'post',
});

// orderByDesc true 降序
const getWindowTab = createRequest<{ id: number; orderByDesc: boolean }, number>('/api/operation/saved/:id', {
  method: 'get',
});
...
const deleteSavedConsole = createRequest<{ id: number }, string>('/api/operation/saved/:id', {
  method: 'delete',
});

const createHistory = createRequest<ISaveBasicInfo, void>('/api/operation/log/create', {
  method: 'post',
});
