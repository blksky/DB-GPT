...
const exportCreateTableSql = createRequest<ITableParams, string>('/api/rdb/ddl/export', {
  method: 'get',
});
const executeTable = createRequest<IExecuteTableParams, string>('/api/rdb/ddl/execute', {
  method: 'post',
});
...
const getKeyList = createRequest<ITableParams, IColumn[]>('/api/rdb/ddl/key_list', {
  method: 'get',
  delayTime: 200,
});
...
const addTablePin = createRequest<IUniversalTableParams, void>('/api/pin/table/add', {
  method: 'post',
});

const deleteTablePin = createRequest<IUniversalTableParams, void>('/api/pin/table/delete', {
  method: 'post',
});

/** 获取当前执行SQL 所有行 */
const getDMLCount = createRequest<IExecuteSqlParams, number>('/api/rdb/dml/count', {
  method: 'post',
});
...
/** 获取修改表数据的接口 */
const getExecuteUpdateSql = createRequest<any, string>('/api/rdb/dml/get_update_sql', {
  method: 'post',
});
