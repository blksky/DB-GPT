...
const zh_tw = {
  translation: {
    ...
    are_you_sure_flush_storage: '您確定要清除儲存空間嗎？這將無法復原地刪除您所有的圖表和自訂範本。',
    ...
    no_values_for_field: "表格 '{{tableName}}' 的 '{{fieldName}}' 欄位型別為 `{{type}}`，但未指定任何值",
    default_doesnt_match_type: "表格 '{{tableName}}' 中欄位 '{{fieldName}}' 的預設值與其型別不符",
    not_null_is_null: "表格 '{{tableName}}' 中的 '{{fieldName}}' 欄位為 NOT NULL，但預設值為 NULL",
    ...
    no_values_for_type_field: "型別 '{{typeName}}' 的 '{{fieldName}}' 欄位型別為 `{{type}}`，但未指定任何值",
    duplicate_type_fields: "自訂型別 '{{typeName}}' 中的欄位名稱 '{{fieldName}}' 重複",
    ...
    pick_db: '選擇資料庫',
    generic: '通用',
    generic_description: '通用圖表可以匯出到任何 SQL 語法，但支援的資料型別較少。',
    enums: '列舉',
    add_enum: '新增列舉',
...
export { traditionalChinese, zh_tw };
