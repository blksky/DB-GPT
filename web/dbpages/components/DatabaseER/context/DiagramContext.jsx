...
export default function DiagramContextProvider({ children }) {
  ...
  const [tableReadonly, setTableReadonly] = useState(true);
  const [dbSchemaInfo, setDbSchemaInfo] = useState(undefined);
  const [generateRelationLoading, setGenerateRelationLoading] = useState(false);
  ...
  const addRelationshipList = (data, addToHistory = true) => {
    if (addToHistory) {
      setRelationships(prev => {
        setUndoStack(prevUndo => [
          ...prevUndo,
      ...
      setRelationships(prev => {
        return [...prev, ...data].map((t, i) => ({ ...t, id: i }));
      });
    }
  };
  const deleteRelationshipList = (list, addToHistory = true) => {
    if (addToHistory) {
      setUndoStack(prev => [
        ...prev,
        {
          ...
          data: [...list],
          message: t('delete_relationship', {
            refName: list.map(d => d.name).join('、'),
          }),
        },
      ]);
      setRedoStack([]);
    ...
    setRelationships(prev => prev.filter(e => !idList.includes(e.id)).map((e, i) => ({ ...e, id: i })));
  };
