...
export function CanvasContextProvider({ children, ...attrs }) {
  ...
  const toDiagramSpace = useCallback(
    coord => ({
      x: typeof coord.x === 'number' ? (coord.x / screenSize.x) * viewBox.width + viewBox.left : undefined,
      y: typeof coord.y === 'number' ? (coord.y / screenSize.y) * viewBox.height + viewBox.top : undefined,
    }),
  ...
  const toScreenSpace = useCallback(
    coord => ({
      x: typeof coord.x === 'number' ? ((coord.x - viewBox.left) / viewBox.width) * screenSize.x : undefined,
      y: typeof coord.y === 'number' ? ((coord.y - viewBox.top) / viewBox.height) * screenSize.y : undefined,
    }),
    [screenSize.x, screenSize.y, viewBox.height, viewBox.left, viewBox.top, viewBox.width],
  );
