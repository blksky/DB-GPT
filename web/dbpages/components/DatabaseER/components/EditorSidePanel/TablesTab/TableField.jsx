import { IconKeyStroked, IconMore } from '@douyinfe/semi-icons';
import { Button, Input, Popover, Select, TextArea } from '@douyinfe/semi-ui';
...
export default function TableField({ data, tid, index }) {
  ...
  const { tables, database, tableReadonly } = useDiagram();
  const { t } = useTranslation();
  const { setUndoStack, setRedoStack } = useUndoRedo();
  const [editField, setEditField] = useState({});

  return (
    <Flex gap={5} align='center' className='hover-1 my-2'>
      <Input
        style={{ flex: 0.5 }}
