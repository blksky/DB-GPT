import DatabaseDocIndex from '@/pages/dbpages/knowledge/databaseDoc';
import GoldenSqlIndex from '@/pages/dbpages/knowledge/goldSql';
import { IconDeleteStroked } from '@douyinfe/semi-icons';
import { Button, Card, Collapse, Input, Popover, TextArea } from '@douyinfe/semi-ui';
...
export default function TableInfo({ data }) {
  ...
  const { deleteTable, updateTable, updateField, setRelationships, database, tableReadonly, dbSchemaInfo } =
    useDiagram();
        ...
        {data.fields.map((f, j) => (
          <div
            ...
            onDragLeave={() => {
              setDrag(prev => ({
                ...prev,
                draggingOverIndexList: prev.draggingOverIndexList.filter(index => index !== j),
              }));
            }}
            onDragOver={e => {
              e.preventDefault();
    ...
    items: [
      {
        key: 'goldSql',
        label: 'Gold SQL',
        children: <GoldenSqlIndex tableName={data.name} dbSchemaInfo={dbSchemaInfo} />,
      },
      {
        key: 'descFile',
        label: '表文档',
        children: <DatabaseDocIndex tableName={data.name} dbSchemaInfo={dbSchemaInfo} />,
      },
      { key: 'field', label: '列信息', children: renderFieldList() },
    ],
  };
  ...
  return (
    <div>
      ...
      <Flex align='center' gap={5} style={{ marginTop: '12px', marginBottom: '12px' }}>
