...
import { Button, Checkbox, Input, InputNumber, TagInput, TextArea } from '@douyinfe/semi-ui';
...
export default function FieldDetails({ data, tid, index }) {
  ...
  const { updateField, deleteField, tableReadonly } = useDiagram();
  ...
  return (
    <div>
      ...
      <Input
        ...
        value={data.default}
        disabled={dbToTypes[database][data.type].noDefault || data.increment || tableReadonly}
      ...
      {dbToTypes[database][data.type].hasCheck && (
        <>
          ...
          <Input
            ...
            placeholder={t('check')}
            value={data.check}
            disabled={data.increment || tableReadonly}
      ...
      <div className='flex justify-between items-center my-3'>
        ...
        <Checkbox
          ...
          disabled={!dbToTypes[database][data.type].canIncrement || data.isArray || tableReadonly}
          onChange={checkedValues => {
            setUndoStack(prev => [
              ...prev,
              {
