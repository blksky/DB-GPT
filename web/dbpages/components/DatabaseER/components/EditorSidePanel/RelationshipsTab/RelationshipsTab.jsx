  ...
  return (
    <>
      ...
      {relationships.length <= 0 ? (
        ...
        <Collapse
          activeKey={
            selectedElement.open && selectedElement.element === ObjectType.RELATIONSHIP ? `${selectedElement.id}` : ''
          }
          keepDOM
          lazyRender
          onChange={k =>
        ...
        >
          {relationships.map(r => (
            <div id={`scroll_ref_${r.id}`} key={'relationship_' + r.id}>
              <Collapse.Panel
                header={<div className='overflow-hidden text-ellipsis whitespace-nowrap'>{r.name}</div>}
                itemKey={`${r.id}`}
              >
