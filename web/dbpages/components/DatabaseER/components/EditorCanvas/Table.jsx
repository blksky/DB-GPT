...
import { ObjectType, Tab, tableColorStripHeight, tableFieldHeight, tableHeaderHeight } from '../../data/constants';
...
export default function Table(props) {
  ...
  const { tableData, onPointerDown, setHoveredTable, handleGripField, setLinkingLine } = props;
  const { layout } = useLayout();
  const { deleteTable, deleteField, tableReadonly, dbSchemaInfo } = useDiagram();
  ...
  const openConsole = () => {
    createConsole({
      dataSourceId: dbSchemaInfo.dataSourceId,
      dataSourceName: dbSchemaInfo.dataSourceName,
      databaseType: dbSchemaInfo.databaseType,
      databaseName: dbSchemaInfo.dataSourceName,
      schemaName: dbSchemaInfo.dataSchemaName,
      executeOnInit: true,
      ddl: `SELECT * from \`${dbSchemaInfo.dataSchemaName}\`.\`${tableData.name}\``,
    });
  };
    ...
    <>
      ...
      >
        <div
          ...
          className={`border-2 hover:border-dashed hover:border-blue-500
               select-none rounded-lg w-full ${
                 settings.mode === 'light' ? 'bg-zinc-100 text-zinc-800' : 'bg-zinc-800 text-zinc-200'
               } ${
        ...
        >
          ...
          >
            <div className=' px-3 overflow-hidden text-ellipsis whitespace-nowrap'>{tableData.name}</div>
                ...
                <Popover
                  ...
                  content={
                    <div className='popover-theme'>
                      ...
                      <div>
                        <strong className={`${tableData.indices.length === 0 ? '' : 'block'}`}>{t('indices')}:</strong>{' '}
          ...
          {tableData.fields.map((e, i) => {
            return settings.showFieldSummary ? (
              <Popover
                ...
                content={
                  <div className='popover-theme'>
                    <div className='flex justify-between items-center pb-2' style={{ direction: 'ltr' }}>
                      <p className='me-4 font-bold'>{e.name}</p>
                      <p className='ms-4'>
                        {e.type +
                          ((dbToTypes[database][e.type].isSized || dbToTypes[database][e.type].hasPrecision) &&
                          e.size &&
                          e.size !== ''
                            ? '(' + e.size + ')'
                            : '')}
    ...
    return (
      ...
      >
        ...
        <div className='text-zinc-400'>
          {hoveredField === index && !tableReadonly ? (
            <Button
              theme='solid'
              size='small'
              style={{
            ...
            <div className='flex gap-1 items-center'>
              ...
              <span>
                {fieldData.type +
                  ((dbToTypes[database][fieldData.type].isSized || dbToTypes[database][fieldData.type].hasPrecision) &&
                  fieldData.size &&
                  fieldData.size !== ''
                    ? '(' + fieldData.size + ')'
                    : '')}
