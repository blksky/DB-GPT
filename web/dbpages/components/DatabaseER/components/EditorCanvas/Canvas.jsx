...
export default function Canvas() {
  ...
  const handlePointerMove = e => {
    ...
    if (linking) {
    ...
    } else if (panning.isPanning && dragging.element === ObjectType.NONE && areaResize.id === -1) {
      if (!settings.panning) {
        return;
      }
      setTransform(prev => ({
        ...prev,
        pan: {
          x: panning.panStart.x + (panning.cursorStart.x - pointer.spaces.screen.x) / transform.zoom,
          y: panning.panStart.y + (panning.cursorStart.y - pointer.spaces.screen.y) / transform.zoom,
        },
      }));
    ...
    } else if (dragging.element === ObjectType.AREA && dragging.id >= 0 && areaResize.id === -1) {
      ...
      switch (areaResize.dir) {
        ...
        case 'tl':
          ...
          newDims.width = initCoords.x + initCoords.width - pointer.spaces.diagram.x;
          newDims.height = initCoords.y + initCoords.height - pointer.spaces.diagram.y;
          break;
        case 'tr':
          newDims.y = pointer.spaces.diagram.y;
          newDims.width = pointer.spaces.diagram.x - initCoords.x;
  ...
  const coordsDidUpdate = element => {
    switch (element) {
      case ObjectType.TABLE:
        return !(dragging.prevX === tables[dragging.id].x && dragging.prevY === tables[dragging.id].y);
      case ObjectType.AREA:
        return !(dragging.prevX === areas[dragging.id].x && dragging.prevY === areas[dragging.id].y);
      case ObjectType.NOTE:
        return !(dragging.prevX === notes[dragging.id].x && dragging.prevY === notes[dragging.id].y);
      default:
        return false;
    }
  };
  ...
  const didPan = () => !(transform.pan.x === panning.x && transform.pan.y === panning.y);
  ...
  const handleLinking = () => {
    ...
    if (linkingLine.startTableId === hoveredTable.tableId && linkingLine.startFieldId === hoveredTable.field) return;
  ...
  useEventListener(
    ...
    e => {
      ...
      if (e.ctrlKey) {
        ...
        setTransform(prev => ({
          pan: {
            x: prev.pan.x - (pointer.spaces.diagram.x - prev.pan.x) * eagernessFactor * Math.sign(e.deltaY),
            y: prev.pan.y - (pointer.spaces.diagram.y - prev.pan.y) * eagernessFactor * Math.sign(e.deltaY),
          },
        ...
        {settings.showGrid && (
          <svg className='absolute w-full h-full'>
            <defs>
              ...
              >
                <circle id='pattern-circle' cx='4' cy='4' r='0.85' fill='rgb(99, 152, 191)'></circle>
              </pattern>
            </defs>
        ...
        >
          {areas?.map(a => (
            <Area
              key={a.id}
              data={a}
              onPointerDown={e => handlePointerDownOnElement(e, a.id, ObjectType.AREA)}
          ...
          {tables?.map(table => (
            <Table
              ...
              onPointerDown={e => handlePointerDownOnElement(e, table.id, ObjectType.TABLE)}
            />
          ))}
          {linking && (
            <path
          ...
          {notes?.map(n => (
            <Note key={n.id} data={n} onPointerDown={e => handlePointerDownOnElement(e, n.id, ObjectType.NOTE)} />
          ))}
        </svg>
      </div>
