import { IconCheckboxTick, IconDeleteStroked, IconEdit } from '@douyinfe/semi-icons';
import { Button, Input, Popover } from '@douyinfe/semi-ui';
import { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { Action, ObjectType, State, Tab, noteThemes } from '../../data/constants';
import { useLayout, useNotes, useSaveState, useSelect, useUndoRedo } from '../../hooks';
    ...
    >
      <path
        d={`M${data.x + fold} ${data.y} L${data.x + w - r} ${
          ...
          data.x + r
        } ${data.y + data.height} A${r} ${r} 0 0 1 ${data.x} ${data.y + data.height - r} L${data.x} ${data.y + fold}`}
        fill={data.color}
        stroke={
          hovered
            ? 'rgb(59 130 246)'
            : selectedElement.element === ObjectType.NOTE && selectedElement.id === data.id
              ? 'rgb(59 130 246)'
              : 'rgb(168 162 158)'
        }
      ...
      <path
        ...
        strokeWidth='1.2'
      />
      <foreignObject x={data.x} y={data.y} width={w} height={data.height} onPointerDown={onPointerDown}>
