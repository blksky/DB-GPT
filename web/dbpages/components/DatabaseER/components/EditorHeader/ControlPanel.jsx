import { fetchDeleteDbEr, fetchGenerateRelation } from '@/dbpages/store/dbEr';
...
import { Button, Divider, Dropdown, InputNumber, Popconfirm, Spin, Toast, Tooltip } from '@douyinfe/semi-ui';
...
import { Action, Cardinality, Constraint, DB, MODAL, ObjectType, SIDESHEET, State, Tab } from '../../data/constants';
...
import GoldSqlDialog from '../Dialogs/GoldSqlDialog';
...
export default function ControlPanel({ diagramId, setDiagramId, title, setTitle, lastSaved }) {
  ...
  const {
    ...
    tableReadonly,
    dbSchemaInfo,
    generateRelationLoading,
    setGenerateRelationLoading,
  } = useDiagram();
    ...
    if (a.action === Action.ADD) {
      if (a.element === ObjectType.TABLE) {
        ...
        if (Array.isArray(a.data)) {
          deleteRelationshipList(a.data, false);
        } else {
      ...
      if (a.element === ObjectType.TABLE) {
        ...
        if (Array.isArray(a.data)) {
          addRelationshipList(a.data, false);
        } else {
      ...
      if (a.element === ObjectType.AREA) {
          ...
          setRelationships(prev => {
            ...
            temp = temp.map((e, i) => {
              const recoveredRel = a.data.relationship.find(
                x =>
                  (x.startTableId === e.startTableId && x.startFieldId === e.startFieldId) ||
                  (x.endTableId === e.endTableId && x.endFieldId === a.endFieldId),
              );
              if (e.startTableId === a.tid && e.startFieldId >= a.data.field.id && !recoveredRel) {
                return {
                  ...e,
                  id: i,
              ...
              if (e.endTableId === a.tid && e.endFieldId >= a.data.field.id && !recoveredRel) {
                return {
                  ...e,
                  id: i,
        ...
        if (Array.isArray(a.rid)) {
          setRelationships(prev =>
            prev.map((e, idx) => {
              return a.rid.includes(idx) ? { ...e, ...a.undo[idx] } : e;
            }),
          );
        } else {
        ...
        if (a.component === 'field_add') {
          updateType(a.tid, {
            fields: types[a.tid].fields.filter((_, i) => i !== types[a.tid].fields.length - 1),
          });
        }
        if (a.component === 'field') {
          updateType(a.tid, {
          ...
          if (a.updatedFields) {
            if (a.undo.name) {
              a.updatedFields.forEach(x => updateField(x.tid, x.fid, { type: a.undo.name.toUpperCase() }));
            }
          }
        }
      } else if (a.element === ObjectType.ENUM) {
    ...
    if (a.action === Action.ADD) {
      ...
      if (a.element === ObjectType.AREA) {
        ...
        if (a.component === 'field') {
          ...
          updateTable(a.tid, {
            indices: tables[a.tid].indices.filter(e => e.id !== a.data.id).map((t, i) => ({ ...t, id: i })),
          });
        } else if (a.component === 'self') {
        ...
        if (Array.isArray(a.rid)) {
          setRelationships(prev =>
            prev.map((e, idx) => {
              return a.rid.includes(idx) ? { ...e, ...a.redo[idx] } : e;
            }),
          );
        } else {
        ...
        if (a.component === 'field_add') {
          ...
          if (a.updatedFields) {
            if (a.redo.name) {
              a.updatedFields.forEach(x => updateField(x.tid, x.fid, { type: a.redo.name.toUpperCase() }));
            }
          }
        }
      } else if (a.element === ObjectType.ENUM) {
  ...
  const showGoldSql = () => {
    refGoldSqlDialog.current?.setOpen(true);
  };

  const handleGenerateRelation = async () => {
    const data_source_name = dbSchemaInfo.dataSourceName;
    const data_schema_name = dbSchemaInfo.dataSchemaName;
    if (!data_source_name || !data_schema_name) return false;
    setGenerateRelationLoading(true);
    // 将之前生成的但未确认的关系删除
    deleteRelationshipList(relationships.filter(d => d.shouldConfirm === true));

    const relations = await fetchGenerateRelation({ data_source_name, data_schema_name });
    const tableNameMap = new Map(tables.map(d => [d.name, d]));
    const relationAddList = [];
    const relationKeyList = relationships.map(
      d => `${d.startTableId}.${d.startFieldId}->${d.endTableId}.${d.endFieldId}`,
    );
    relations.forEach(relation => {
      let [sourceTableName, sourceColumn] = relation.source_column.split('.');
      let [targetTableName, targetColumn] = relation.target_column.split('.');
      const directionOk =
        sourceColumn.toLowerCase().includes(sourceTableName.toLowerCase()) ||
        sourceTableName.toLowerCase().includes(sourceColumn.toLowerCase());
      if (!directionOk) {
        [sourceColumn, targetColumn] = [targetColumn, sourceColumn];
        [sourceTableName, targetTableName] = [targetTableName, sourceTableName];
      }

      const sourceTable = tableNameMap.get(sourceTableName);
      const targetTable = tableNameMap.get(targetTableName);
      const sourceField = sourceTable?.fields.find(d => d.name === sourceColumn);
      const targetField = targetTable?.fields.find(d => d.name === targetColumn);

      if (sourceField && targetField && sourceField.type === targetField.type) {
        const relationKey = `${sourceTable.id}.${sourceField.id}->${targetTable.id}.${targetField.id}`;
        if (!relationKeyList.includes(relationKey)) {
          const newRelationship = {
            ...
            name: `${sourceTable.name}_${sourceField.name}_fk`,
            id: relationships.length + relationAddList.length,
          };
          relationAddList.push(newRelationship);
        }
      }
    });
    if (!!relationAddList.length) {
      addRelationshipList(relationAddList);
    }
    setGenerateRelationLoading(false);
  };

  const handleGenerateRelationConfirm = confirm => {
    if (confirm) {
      const confirmList = relationships.filter(d => d.shouldConfirm);

      setUndoStack(prev => {
        const undoMap = {};
        const redoMap = {};
        confirmList.forEach(d => {
          undoMap[d.id] = { shouldConfirm: d.shouldConfirm };
          redoMap[d.id] = { shouldConfirm: false };
        });
        return [
          ...prev,
          {
            ...
            rid: confirmList.map(d => d.id),
            undo: undoMap,
            redo: redoMap,
            message: t('edit_relationship', {
              refName: confirmList.map(d => d.name).join(','),
              extra: '[shouldConfirm]',
            }),
          },
        ];
      ...
      setRedoStack([]);
      setRelationships(prev => {
        prev.forEach(d => (d.shouldConfirm = false));
        return [...prev];
      });
    } else {
      deleteRelationshipList(relationships.filter(d => d.shouldConfirm === true));
  ...
  const menu = {
    file: {
      ...
      delete_diagram: {
        ...
        function: async () => {
          const result = await fetchDeleteDbEr({ id: diagramId });
          if (result) {
            setDiagramId(0);
      ...
      exit: {
        function: () => {
          save();
          if (saveState === State.SAVED) router.push('/');
        },
      },
    },
    edit: {
    ...
    view: {
      header: {
        state: layout.header ? <i className='bi bi-toggle-on' /> : <i className='bi bi-toggle-off' />,
        function: () => setLayout(prev => ({ ...prev, header: !prev.header })),
      },
      sidebar: {
        state: layout.sidebar ? <i className='bi bi-toggle-on' /> : <i className='bi bi-toggle-off' />,
        function: () => setLayout(prev => ({ ...prev, sidebar: !prev.sidebar })),
      },
      issues: {
        state: layout.issues ? <i className='bi bi-toggle-on' /> : <i className='bi bi-toggle-off' />,
        function: () => setLayout(prev => ({ ...prev, issues: !prev.issues })),
      },
      strict_mode: {
        state: settings.strictMode ? <i className='bi bi-toggle-off' /> : <i className='bi bi-toggle-on' />,
      ...
      field_details: {
        state: settings.showFieldSummary ? <i className='bi bi-toggle-on' /> : <i className='bi bi-toggle-off' />,
      ...
      show_debug_coordinates: {
        state: settings.showDebugCoordinates ? <i className='bi bi-toggle-on' /> : <i className='bi bi-toggle-off' />,
        function: () =>
          setSettings(prev => ({
            ...prev,
  ...
  return (
    <>
      ...
      <GoldSqlDialog ref={refGoldSqlDialog} dbSchemaInfo={dbSchemaInfo} />
    </>
  );

  function toolbar() {
    const hanRelationshipShouldConfirm = relationships.some(d => d.shouldConfirm);
    ...
    return (
      <div
        ...
        className='py-1.5 px-5 flex justify-between items-center my-1 sm:mx-1 select-none overflow-hidden toolbar-theme'
        style={isRtl(i18n.language) ? { direction: 'rtl' } : {}}
      >
        <div className='flex justify-start items-center'>
          ...
          <Tooltip content='查看Gold SQL' position='bottom'>
            <button className='py-1 px-2 hover-2 rounded flex items-center' onClick={showGoldSql}>
              <IconFont type='icon-chat-gold_sql' className='text-24px' />
            </button>
          </Tooltip>
          {!hanRelationshipShouldConfirm && (
            <Tooltip content='一键生成关联关系' position='bottom'>
              <button className='py-1 px-2 hover-2 rounded flex items-center' onClick={handleGenerateRelation}>
                {generateRelationLoading ? (
                  <LoadingOutlined className='text-24px' />
                ) : (
                  <BulbOutlined className='text-24px' />
                )}
              </button>
            </Tooltip>
          )}
          {hanRelationshipShouldConfirm && (
            <Tooltip content='一键确认生成的关联关系' position='bottom'>
              <button
                className='py-1 px-2 hover-2 rounded flex items-center'
                onClick={() => handleGenerateRelationConfirm(true)}
              >
                <CheckOutlined />
              </button>
            </Tooltip>
          ...
          {hanRelationshipShouldConfirm && (
            <Tooltip content='一键移除生成的关联关系' position='bottom'>
              <button
                className='py-1 px-2 hover-2 rounded flex items-center'
                onClick={() => handleGenerateRelationConfirm(false)}
              >
                <CloseOutlined />
              </button>
            </Tooltip>
            ...
            <div className='flex justify-between items-center'>
              <div className='flex justify-start text-md select-none me-2'>
                {Object.keys(menu).map(category => (
                            ...
                            return (
                              ...
                              >
                                ...
                                >
                                  ...
                                  {isRtl(i18n.language) ? <IconChevronLeft /> : <IconChevronRight />}
                                </Dropdown.Item>
                              </Dropdown>
                            );
                          }
                            ...
                            >
                              <div className='w-full flex items-center justify-between'>
                                ...
                                <div className='flex items-center gap-1'>
                                  {menu[category][item].shortcut && (
                                    <div className='text-gray-400'>{menu[category][item].shortcut}</div>
                                  )}
                  ...
                  >
                    <div className='px-3 py-1 hover-2 rounded'>{t(category)}</div>
                  </Dropdown>
                ))}
              </div>
              <Button
                size='small'
                type='tertiary'
                icon={saveState === State.LOADING || saveState === State.SAVING ? <Spin size='small' /> : null}
              >
                {getState()}
              </Button>
            </div>
