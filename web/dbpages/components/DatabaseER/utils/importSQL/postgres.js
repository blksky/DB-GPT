...
const affinity = {
  [DB.POSTGRES]: new Proxy({ INT: 'INTEGER' }, { get: (target, prop) => (prop in target ? target[prop] : 'BLOB') }),
...
export function fromPostgres(ast, diagramDb = DB.GENERIC) {
  ...
  const parseSingleStatement = e => {
    if (e.type === 'create') {
        ...
        e.create_definitions.forEach(d => {
          ...
          if (d.resource === 'column') {
            ...
            if (d.constraint_type === 'primary key') {
              ...
              const endField = d.reference_definition.definition[0].column.expr.value;
      ...
      e.expr.forEach(expr => {
        if (expr.action === 'add' && expr.create_definitions.constraint_type.toLowerCase() === 'foreign key') {
          const relationship = {};
          const startTable = e.table[0].table;
          const startField = expr.create_definitions.definition[0].column.expr.value;
          const endTable = expr.create_definitions.reference_definition.table[0].table;
          const endField = expr.create_definitions.reference_definition.definition[0].column.expr.value;
          let updateConstraint = 'No action';
          let deleteConstraint = 'No action';
