...
const affinity = {
  [DB.MARIADB]: new Proxy({ INT: 'INTEGER' }, { get: (target, prop) => (prop in target ? target[prop] : 'BLOB') }),
    ...
    if (e.type === 'create') {
      ...
      e.expr.forEach(expr => {
        if (expr.action === 'add' && expr.create_definitions.constraint_type.toLowerCase() === 'foreign key') {
          ...
          const endField = expr.create_definitions.reference_definition.definition[0].column;
          let updateConstraint = 'No action';
          let deleteConstraint = 'No action';
