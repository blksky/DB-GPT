...
export function fromSQLite(ast, diagramDb = DB.GENERIC) {
  ...
  const addRelationshipFromReferenceDef = (startTable, startFieldName, referenceDefinition) => {
    ...
    const startFieldId = startTable.fields.findIndex(f => f.name === startFieldName);
        ...
        e.create_definitions.forEach(d => {
          if (d.resource === 'column') {
            ...
            if (d.constraint_type === 'primary key') {
              ...
              addRelationshipFromReferenceDef(table, d.definition[0].column, d.reference_definition);
            }
          }
        });
