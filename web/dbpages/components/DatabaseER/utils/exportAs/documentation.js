...
export function jsonToDocumentation(obj) {
    ...
    .map(table => {
      ...
      const fields = table.fields
        .map(field => {
          const fieldType =
            field.type +
            ((dbToTypes[obj.database][field.type].isSized || dbToTypes[obj.database][field.type].hasPrecision) &&
            field.size &&
            field.size !== ''
              ? '(' + field.size + ')'
              : '');
  ...
  function relationshipByField(table, relationships, fieldId) {
    return relationships.filter(r => r.startTableId === table && r.startFieldId === fieldId).map(rel => rel.name);
  }
