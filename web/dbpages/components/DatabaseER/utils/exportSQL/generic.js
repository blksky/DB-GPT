...
export function getTypeString(field, currentDb, dbms = 'mysql', baseType = false) {
  if (dbms === 'mysql') {
    if (field.type === 'UUID') {
      return `VARCHAR(36)`;
    }
    ...
    if (dbToTypes[currentDb][field.type].isSized) {
      const type =
        field.type === 'BINARY' ? 'bit' : field.type === 'VARBINARY' ? 'bit varying' : field.type.toLowerCase();
    ...
    switch (field.type) {
      case 'ENUM':
        return baseType
          ? 'NVARCHAR(255)'
          : `NVARCHAR(255) CHECK([${field.name}] in (${field.values.map(v => `'${v}'`).join(', ')}))`;
      case 'VARCHAR':
        type = `NVARCHAR`;
        break;
      case 'UUID':
    ...
    .map(
      table =>
        `CREATE TABLE \`${table.name}\` (\n${table.fields
          .map(
            field =>
              `\t\`${field.name}\` ${getTypeString(field, obj.database)}${field.notNull ? ' NOT NULL' : ''}${
...
export function jsonToPostgreSQL(obj) {
  return `${obj.types.map(type => {
    const typeStatements = type.fields
      ...
      .map(f => `CREATE TYPE "${f.name}_t" AS ENUM (${f.values.map(v => `'${v}'`).join(', ')});`)
      .join('\n');
    ...
    .map(
      table =>
        `${
          ...
          .map(
            field =>
              `${field.comment === '' ? '' : `\t-- ${field.comment}\n`}\t"${
                ...
                field.check === '' || !dbToTypes[obj.database][field.type].hasCheck ? '' : ` CHECK(${field.check})`
              }`,
          )
          .join(',\n')}${
          table.fields.filter(f => f.primary).length > 0
          ...
          .map(field =>
            field.comment.trim() !== '' ? `COMMENT ON COLUMN ${table.name}.${field.name} IS '${field.comment}';\n` : '',
          )
          .join('')}\n${table.indices
          .map(
            i =>
  ...
  return `${obj.types
    .map(type => {
      return `${type.comment === '' ? '' : `/**\n${type.comment}\n*/\n`}CREATE TYPE [${type.name}] FROM ${
        type.fields.length < 0 ? '' : `${getTypeString(type.fields[0], obj.database, 'mssql', true)}`
      };\nGO\n`;
    })
    .join('\n')}\n${obj.tables
    .map(
