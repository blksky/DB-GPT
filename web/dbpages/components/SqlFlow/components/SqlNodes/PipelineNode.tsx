...
const PipelineNode: FC<{ node: Node; graph: Graph }> = ({ node }) => {
  const { label, isModel, stepData, isDatabase, className, databaseType = 'SQLITE' } = node.getData() as INodeData;
  ...
  return (
    <Flex
      ...
      justify='center'
      align='center'
      className={classNames('sql-flow-node', 'pipeline-node', className, stepData?.status)}
    >
      {nodeIcon}
      {label}
