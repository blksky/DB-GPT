...
export function osNow(): {
  ...
  const agent = typeof window !== 'undefined' ? navigator.userAgent.toLowerCase() : '';
  const isMac = typeof window !== 'undefined' ? /macintosh|mac os x/i.test(navigator.userAgent) : false;
  const isWin =
