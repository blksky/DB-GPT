import { AUTH_TOKEN_KEY, NumericUnit } from '@/dbpages/common/constants';
...
export const isMobile = typeof window !== 'undefined' && window.navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i);
...
export const getFormattedValueData = (value: number | string, remainZero?: boolean) => {
  ...
  const unit =
    value >= 100000000 ? NumericUnit.OneHundredMillion : value >= 10000 ? NumericUnit.EnTenThousand : NumericUnit.None;
...
export function traverseRoutes(routes, env: string, result: any[] = []) {
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    if (route.routes) {
      ...
      (route.envEnableList && (route.envEnableList.includes(env) || route.envEnableList.length === 0)) ||
      !route.envEnableList
    ) {
      result.push(route);
    }
...
const encryptKey = CryptoJS.enc.Hex.parse('9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08');
...
export function formatMsTime(mss: number, withMs = false) {
  const day = Math.floor(mss / (1000 * 60 * 60 * 24));
  const hour = Math.floor((mss % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
  const minute = Math.floor((mss % (1000 * 60 * 60)) / (1000 * 60));
  const second = Math.floor((mss % (1000 * 60)) / 1000);
  const milliSecond = mss % 1000;
  ...
  if (withMs && milliSecond >= 1) {
    result.push(`${milliSecond}ms`);
  }
  if (!result.length) {
    return `0s`;
  }
